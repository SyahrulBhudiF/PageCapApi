...
func RegisterAuthRoutes(rg *gin.RouterGroup, authHandler *handler.AuthHandler, mm *midleware.AuthMiddleware) {
	auth := rg.Group("/auth")
	{
		...
		auth.POST("/refresh", mm.EnsureAuthenticated(), authHandler.RefreshToken)
		...
		auth.POST("/google/verify", midleware.EnsureJsonValidRequest[dto.GoogleVerifyRequest](), authHandler.GoogleVerifyToken)
